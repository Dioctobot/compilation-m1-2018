...
and value_definition env = function
  ...
  | HopixAST.RecFunctions fs ->
     HobixAST.RecFunctions (List.map (function_definition env) fs)

and function_definition env (f, _, HopixAST.FunctionDefinition (xs, e)) =
  let xs = List.map (located identifier) xs in
...
and expression env = HobixAST.(function
  ...
  | HopixAST.IfThenElse (c, t, f) ->
     let f =
       ...
       | Some f -> located (expression env) f
     in
  ...
  | HopixAST.For (x, start, stop, step, e) ->
    failwith "Students! This is your job!"

  | HopixAST.Fun (HopixAST.FunctionDefinition (xs, e)) ->
    failwith "Students! This is your job!"
)
