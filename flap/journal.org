...
** de Hopix à Hobix
   - C(K(e₁, ⋯, eₙ)) =
     construire un bloc de taille n+1 et utiliser un (petit) entier
     pour représenter l'étiquette K.
   - C(case e of p₁ → e₁ | ⋯ | pₙ → eₙ) =
...
** Vue d'ensemble des optimisations à venir
   ...
   - tail recursion
   - analyse CFA
   - factorisation des sous-expressions communes
   - propagation des constantes + déroulage
...
if z = T then
   if y = T then 4 else 1
else (* z = F *)
   if x = F then 

*** Article de Luc Maranget
**** TODO Lire http://moscova.inria.fr/~maranget/papers/ml05e-maranget.pdf

** Compilation du switch en X86-64
   - discussion sur les différentes façons d'encoder les tables de saut
     en x86.
* Cours 14 "L'analyse de motifs et l'allocation de registres" <2019-02-18>
** FAQ
** TD: Spécification du jalon 8 à partir de l'article de Luc Maranget
   - Etablir un plan de développement pour la compilation du pattern-matching
   - Quel est le point où insérer l'appel à la fonction de compilation
     optimisée du pattern-matching?

   - Quels types définir? Quelles fonctions implémenter? Dans quel ordre
     les implémenter et les tester?

   - Le plan de développement:
     1. Développer et tester un module indépendant du compilateur qui implémente
	exactement l'article.
     2. Brancher ce module au compilateur et pour cela:
	a. Généraliser la syntaxe des motifs pour prendre en compte les variables.
	b. Compiler les arbres de décision en Retrolix
...
*** Un exemple
   ...
   Comment faire l'allocation de registres sur cet exemple?
**** Solution
   l1: a <- 100      -> l2
   l2: b <- 0        -> l3
   l3: r3 <- 3       -> l4
...
*** Pourquoi ne peut-on obtenir qu'une approximation du graphe d'interférence?
   Parce que c'est un problème indécidable. Exemple:
...
*** Analyse de vivacité
   ...
   USE(l) l'ensemble des variables (potentiellement) lues par l'instruction en l.
   DEF(l) l'ensemble des variables (potentiellement) modifiées par l'instruction en l.
...
**** Calcul en avant
   ...
   vivantes... mais c'est terriblement long! C'est tellement long que
   je ne suis pas allé jusqu'au bout et que je me suis endormi.
...
* Cours 15
*** TODO Lire http://ithare.com/infographics-operation-costs-in-cpu-clock-cycles/
