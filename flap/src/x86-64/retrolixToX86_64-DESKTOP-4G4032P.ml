...
module FrameManager(IS : InstructionSelector) : FrameManager =
  struct
    ...
    let location_of fd id =
      ...
      let open X86_64_Architecture in
      let index = register_of_string "rip" in
      let idx = register_of_string "rsp" in
      try
        T.addr ~offset:(Lit (S.IdMap.find id fd.stack_map)) 
        ~base:index
        ~idx:idx ()
      with _ -> match id with 
        | Id name -> T.addr ~offset:(Lab name) ~base:index ()
    ...
    let call fd ~kind ~f ~args =
      ...
      let open X86_64_Architecture in
      display f;
      List.iter (fun arg -> display arg) args;
      let rec check_args nb = function
        [] -> []
        | hd::tl -> match nb with
          | _ when nb < 6 ->  
            let reg = (List.nth nb argument_passing_registers) in
            T.(Instruction (movq hd reg))
            ::(check_args tl (nb + 1))
          | _ -> S.IdMap.add (Id ) fd.stack_map
